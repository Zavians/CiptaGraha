...
<nav class="navbar navbar-expand-lg position-absolute top-0 z-index-3 my-3 {{ (Request::is('static-sign-up') ? 'w-100 shadow-none  navbar-transparent mt-4' : 'blur blur-rounded shadow py-2 start-0 end-0 mx4') }}">
  ...
  <a class="nav-link me-2 bg-gradient-info rounded-pill text-white font-weight-bold" href="{{ auth()->user() ? url('static-sign-up') : url('register') }}">
    <i class="fas fa-user-circle opacity-6 me-1 {{ Request::is('static-sign-up') ? 'text-white' : 'text-dark' }}"></i>
    Sign Up
  </a>
</li>
<li class="nav-item">
  <a class="nav-link me-2 bg-gradient-info rounded-pill text-white font-weight-bold" href="{{ auth()->user() ? url('static-sign-in') : url('login') }}">
    <i class="fas fa-key opacity-6 me-1 {{ Request::is('static-sign-in') ? 'text-white' : 'text-dark' }}"></i>
    Sign In
  </a>
</li>
